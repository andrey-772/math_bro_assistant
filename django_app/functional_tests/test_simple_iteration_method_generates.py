...
class TestSimpleIterationSolution(FunctionalTest):
    ...
    def test_not_generates_if_data_is_invalid(self):
        ...
        for item_n in (2, 3, 4, 5):
            ...
            data_set = self.__get_dataset_keys(table_index=f"{item_n}{item_n}")
            data_set2 = self.__get_dataset_keys(table_index=f"{item_n}1")
            ...
            if item_n < 5:
                ...
                data_set2 = self.__get_dataset_keys(table_index=f"{item_n+1}1")
                data_set.extend(data_set2)
                print(data_set)
                for table_index in data_set:
                     field = self.browser.find_element(By.NAME, table_index)
        ...
        for item_n in (2, 3, 4, 5):     
            ...
            if item_n < 5:
                ...
                if row2 == 2:
                    ...
                    fields = self.__get_dataset(table_index=f"{row2}{column2}")
                    for field in fields.keys():
                        self.assertEqual(fields[field], "01")
                elif row2 == 3:
    ...
    def __get_dataset_keys(self, table_index: str) -> list:
        dataset_keys_list = []
